...
import { _generate, _train } from './utils.process'
...
function Graph() {
  ...
  return <div style={{ width: '100%', height: '100%' }} ref={el => ref.current = el}></div>
}
...
function Train() {
  ...
  const train = () => {
    _train(Imitation.state.context, prompt, option)
  }

  return <div style={{ padding: 16 }}>
    <Grid container spacing={2}>
      <Grid item xs={12}>
        <TextField multiline fullWidth value={prompt} onChange={e => setPrompt(e.target.value)} />
      </Grid>
      <Grid item xs={12}>
        <Button variant='outlined' onClick={train}>Train</Button>
      </Grid>
    </Grid>
  </div>
}
...
function App() {
  return <Paper style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', background: 'rgba(255, 255, 255, 1)' }}>
