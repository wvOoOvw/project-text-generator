...
function App() {
  ...
  const [setting, setSetting] = React.useState({ weight: 2, recordContextLengthLeft: 2, recordContextLengthRight: 2, randomAddition: 0, skipBreak: false })
  ...
  const train = async () => {
    ...
    var token = await tokenizerProcessLoop(tokenizer(prompt))

    token = token.filter(i => i !== ' ')
